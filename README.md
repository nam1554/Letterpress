...
작업이 하나도 없고 아직 손볼 백엔드가 남아 있으면 홈 맨 위에 **"처음이신가요?"**
안내가 뜹니다. **"어떤 구독을 갖고 계세요?"** 에서 하나를 고르면 그 구독에
맞는 준비 절차(설치·로그인·Figma 연결)만 명령 복사 버튼과 함께 보여주고,
아래 "새 변환"의 백엔드 선택도 함께 따라갑니다.

### 작업이 실패하면

작업 상세 화면 맨 위에 **"무엇이 문제였나요"** 가 붙습니다. 실패 사유를
읽어 원인(구독 한도 · 로그인 필요 · Figma 접근 · 시간 초과 · 품질 기준
미달 · CLI 미설치)을 분류하고, 지금 할 수 있는 일을 알려줍니다. CLI가 뱉은
영어 원문을 해석할 필요가 없습니다.

가장 흔한 실패인 **구독 한도**는 화면에서 바로 **다른 백엔드로 다시 시도**할
수 있습니다 — 같은 Figma 링크로 새 작업을 만들고, 준비 안 된 백엔드를
고르면 그 사실을 먼저 알려줍니다. 세 백엔드가 같은 합격선을 통과하므로
결과물 수준은 달라지지 않습니다.

원인이 알려진 유형에 맞지 않으면 억지로 지목하지 않고 **"문제 신고용 파일
내려받기"** 를 안내합니다.
...
빠뜨리면 그 구독을 가진 팀원은 첫 화면에서 자기 선택지를 아예 못 찾습니다
(테스트가 잡아냅니다). UI 표시 이름·스모크 테스트·이 문서까지 같이
손봐야 하는 항목입니다. 기본 백엔드는
