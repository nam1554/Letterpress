...
describe("buildEdmPrompt", () => {
  ...
  it("points every backend at the vendored skill copy", () => {
    const p = buildEdmPrompt(base);
    expect(p).toContain(path.join("skills", "figma-edm", "SKILL.md"));
  });

  it("appends the repair appendix with the gate failures", () => {
    const p = buildEdmPrompt({
      ...base,
      repair: { failures: ["output/에 *_figma.html이 없습니다."] },
    });
  ...
  it("switches to the edit prompt when task.edit is set", () => {
    const p = buildEdmPrompt({ ...base, edit: { instruction: "헤드라인을 'B'로 바꿔줘" } });
